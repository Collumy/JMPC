...
def parse_assignment(parser):
    ...
    while True:
        ...
        if parser.current.type != Tokens.ASSIGN: error(parser, "Ожидался = для присваивания")
...
def parse_assign_block(parser):
    ...
    if parser.current.type != Tokens.COLON:
        error(parser, "После 'assign' ожидается ':'")
    parser.eat(Tokens.COLON)

    # newline
    if parser.current.type != Tokens.NEWLINE:
        error(parser, "После ':' ожидается новая строка")
    parser.eat(Tokens.NEWLINE)

    # indent
    if parser.current.type != Tokens.INDENT:
        error(parser, "После новой строки ожидается отступ")
