...
def parse_action(parser, iscustom=False):
    ...
    if parser.current.type != Tokens.SELECTOR and not iscustom:
        ...
        if parser.current.type in (Tokens.DOT, Tokens.DOUBLE_COLON):
            error(parser, "Ожидалась точка или :: для вызова метода")
    ...
    if not is_dot_syntax and not iscustom:
        ...
        target = SELECTORS.get(parser.current.type, parser.current.value)
    ...
    if iscustom:
        parser.eat(Tokens.CUSTOM); parser.eat(Tokens.DOT)
        custom = CUSTOM_ACTIONS[parser.current.value]
