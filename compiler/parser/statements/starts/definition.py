...
VALUES_TYPES = ["any", "text", "number", "location", "vector", "block", "item",
                "sound", "particle", "potion", "list", "map", "variable"]
...
def parse_definition(parser):
    ...
    if parser.current.type == Tokens.LESS:
        ...
        if parser.current.type == Tokens.COMMA:
            parser.eat(Tokens.COMMA)
            desc_expr = parse_expression(parser)
            if not isinstance(desc_expr, TypeString):
                error(parser, "Отображаемое описание должно быть строкой")
            display["description"] = desc_expr
    ...
    if parser.current.type != Tokens.COLON: error(parser, "Ожидалось двоеточие для начала")
...
def advanced_args(parser):
    ...
    while parser.current.type != Tokens.RPAREN:
        ...
        if parser.current.type != Tokens.COLON:
            error(parser, "Ожидалось двоеточие после типа параметра")
        parser.eat(Tokens.COLON)
...
def parse_singular_param(parser, slot_required=False):
    ...
    if parser.current.type == Tokens.LESS:
        ...
        if parser.current.type != Tokens.GREATER: error(parser, "Ожидалось окончание описания параметра")
        parser.eat(Tokens.GREATER)        
...
def parse_plural_param(parser):
    ...
    if parser.current.type != Tokens.LESS: error(parser, "Ожидалось < после названия параметра")
    parser.eat(Tokens.LESS)
    ...
    # Слоты
    if parser.current.type != Tokens.COMMA: error(parser, "Ожидалась , после которой слоты параметра")
    parser.eat(Tokens.COMMA)
    param_slots = parse_list(parser)

    
    # Слот описания
    if parser.current.type != Tokens.COMMA: error(parser, "Ожидалась , после которой слоты описания параметра")
    parser.eat(Tokens.COMMA)
...
def parse_list(parser):
    ...
    if parser.current.type != Tokens.LBRACKET: error(parser, "Ожидалось [ для списка")
    ...
    if parser.current.type != Tokens.RBRACKET: error(parser, "Ожидалось ] для окочнания списка")
...
def parse_enum_param(parser):
    ...
    # Слот
    if parser.current.type != Tokens.COMMA: error(parser, "Ожидалась , после которой слот параметра")
    parser.eat(Tokens.COMMA)
    ...
    if parser.current.type != Tokens.GREATER: error(parser, "Ожидалось > для закрытия описания")
    parser.eat(Tokens.GREATER)

    if parser.current.type != Tokens.COLON: error(parser, "Ожидалось двоеточие для значений маркера")
    parser.eat(Tokens.COLON)
