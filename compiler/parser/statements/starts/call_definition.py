...
from compiler.utils.player.enum_check import get_enum
...
def parse_call(parser):
    if parser.current.type == Tokens.FUNCTION:
        ...
        error(parser, "Ожидалось function или process")
    ...
    if parser.current.type in (Tokens.IDENTIFIER, Tokens.VARIABLE):
        ...
        if kind == "process" and parser.current.type == Tokens.LESS:
            args.extend(parse_proc_info(parser))
...
def parse_proc_info(parser):
    args = []

    parser.eat(Tokens.LESS)
    args.append(Argument("local_variables_mode", get_enum(parser, PROCESS_ENUMS[0])))

    if parser.current.type == Tokens.COMMA:
        parser.eat(Tokens.COMMA)
        args.append(Argument("target_mode", get_enum(parser, PROCESS_ENUMS[1])))
...
def parse_args(params, parser):
    ...
    if parser.current.type == Tokens.RPAREN:
        ...
        for p in params:
            ...
            if p.type_key == "enum": result.append(name, TypeString(p.default))
        return result
    ...
    for p in params:
        ...
        result.append((TypeString(p.name), value))
