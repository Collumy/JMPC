...
from compiler.ast.expressions import TypeEnum, TypeList, TypeVariable, TypeString
...
def parse_all_arguments(parser, arg_metas, context_name="", extra=None):
    ...
    for meta in arg_metas:
            ...
            if meta["type"] == "enum":
                ...
                if isinstance(raw_value, TypeVariable) or isinstance(raw_value, TypeString):
                    text = raw_value.value.upper()
                    short = {i[:2]: i for i in meta["values"]}
                    
                    if text in meta["values"]:
                        args.append(Argument(arg_id, TypeEnum(text))); continue
                    if text in short.keys():
                        args.append(Argument(arg_id, TypeEnum(short[text]))); continue
                ...
                error(parser,
                    f"Недопустимое значение '{raw_value.value}' для аргумента '{arg_id}'. "
