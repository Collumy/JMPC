...
from compiler.lexer.keywords import KEYWORDS, VAR_PREFIXES
        ...
        while self.current_char is not None:
            ...
            if self.current_char.isalpha() or self.current_char == "_":
                ...
                if ident in KEYWORDS:
                    return Token(KEYWORDS[ident], ident, self.line, self.column)
                
                if self.current_char == "(":
                    return Token(Tokens.IDENTIFIER, ident, self.line, self.column)
                
                if self.current_char == ":" and self.peek() == ":":
                    return Token(Tokens.SELECTOR, ident, self.line, self.column)
                
                if ident in VAR_PREFIXES and self.current_char == "`":
                    var_type = VAR_PREFIXES[ident]
                    name = self.make_backtick_variable()
